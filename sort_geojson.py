...
def split_track_by_stations_shapely(xs, ys, stations):
    line = LineString(list(zip(xs, ys)))
...
def get_unordered_stations(target_refs):
    stations = []
...
def get_unordered_segments(target_refs):
    segments = []
    for feature in data["features"]:
        geom = feature["geometry"]
        props = feature["properties"]
...
segments_unordered_L2 = get_unordered_segments("L2")
segments_unordered_L3 = get_unordered_segments("L3")

# -----------------------------
# Ordered coordinates
# -----------------------------
L2_ordered = to_ordered_coords(segments_unordered_L2)[0] #0 is for tracks bound for Randwick, 1 is for tracks bound for Circular Quay
L3_ordered = to_ordered_coords(segments_unordered_L3)[1] #0 is for tracks bound for Circular Quay, 1 is for tracks bound for Juniors Kingsford, 
lons = [coord[0] for coord in (L2_ordered + L3_ordered)]
lats = [coord[1] for coord in (L2_ordered + L3_ordered)]

origin = ((np.min(lons) + np.max(lons))/2, (np.min(lats) + np.max(lats))/2)
xs_L2, ys_L2 = degrees_to_metres(*zip(*L2_ordered), origin)
xs_L3, ys_L3 = degrees_to_metres(*zip(*L3_ordered), origin)

stations_L2 = []
for lon, lat, name in get_unordered_stations("L2"):
    stations_L2.append((*degrees_to_metres(lon, lat, origin), name))
stations_L3 = []
for lon, lat, name in get_unordered_stations("L3"):
